...
<script >
  export default {
    ...
    data () {
      return {
        btntxt: '获取验证码',
        register_fast:login_style,          //登录方式
        hide:false,                         //删除账号按钮开关
    ...
    methods:{
      ...
      loginpost:function () {
         if(this.register_fast){ //验证码登录
           this.$emit('load',true)
//          this.$axios.get(this.code_url_btn+'?mobile='+this.phone+'&code='+this.Codetxt).then(res=>{
           this.$axios.post(this.code_url_btn,this.qs.stringify({
            ...
            this.$emit('load',true)
//            this.$axios.get(this.login_url+'?username='+this.phone+'&password='+this.password+'&isCard=1&regFrom=2')
      ...
      //获取验证码请求
      ajax:function () {
//        this.$axios.get(this.code_url+'?mobile='+this.phone+'&type=3')
